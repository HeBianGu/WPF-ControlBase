...
{
    [ViewModel("TreeList")]
    internal partial class TreeListViewModel : MvcViewModelBase
    {
        private int _count;
        /// <summary> 说明  </summary>
        public int Count
        ...
        public void InitTyeEncodeDevice(List<TreeNodeEntity> uses)
        {
            Task.Run(() =>
            {
                ...
                foreach (TreeNodeEntity item in uses)
                {
                ...
                //  Message：触发筛选
                RefreshNodes(FilterText, SelectEntityNode);
            });
        ...
        public void LoadTyeEncodeCheckDevice(List<TreeNodeEntity> uses)
        {
            ...
            TreeNodeEntityViewModel all = new TreeNodeEntityViewModel(new TreeNodeEntity() { Name = "全部" })
            {
                IsSelected = true
            };
        ...
        private void RefreshCount()
        {
            ...
            action = k =>
            {
                foreach (TreeNodeEntityViewModel item in k)
                {
                    //  Do：匹配则显示
        ...
        {
            RefreshNodes(text, SelectEntityNode);
        }

        private List<TreeNodeEntityViewModel> matchNodes = new List<TreeNodeEntityViewModel>();
        ...
        public void RefreshNodes(string text, TreeNodeEntityViewModel selectNode)
        {
            ...
            foreach (TreeNodeEntityViewModel item in Nodes)
            {
                //  Do：先检查第一级别过滤设备节点
            ...
            if (!string.IsNullOrEmpty(text) && matchNodes.Count > 0)
            {
                ...
                Action<TreeNodeEntityViewModel> CloseChildChild = l =>
                {
                    ...
                    foreach (TreeNodeEntityViewModel item in l.Nodes)
                    {
                        if (item.Nodes == null) return;

                        foreach (TreeNodeEntityViewModel item1 in item.Nodes)
                        {
                ...
                if (isCode)
                {
                    ...
                    TreeNodeEntityViewModel find = matchNodes.Find(l => l.Code == text.TrimEnd('0'));
                ...
                else
                {
                    ...
                    IEnumerable<TreeNodeEntityViewModel> collection = matchNodes.Take(500);

                    foreach (TreeNodeEntityViewModel item in matchNodes)
                    {
                        ExpandParent(item);
                    }
        ...
        protected override async void RelayMethod(object obj)
        {
            ...
            else if (command == "TreeList.Load")
            {
                await Message.Instance.ShowWaittingMessge(() =>
                {
                    ...
                    List<TreeNodeEntity> collecion = JsonConvert.DeserializeObject<List<TreeNodeEntity>>(txt);

                    //  Message：初始化树形控件（只需初始化一遍）
                    InitTyeEncodeDevice(collecion);

                    LoadTyeEncodeCheckDevice(collecion.Where(l => l.Code.Length == 2).ToList());
