...
namespace HeBianGu.Window.MessageDialog
{
    ...
    /// </summary>
    public partial class MessageDialogWindow : IMessageDialogWindow
    {
        ...
        public static bool ShowDialog(string messge, string title = null, int closeTime = -1, bool showEffect = true, params Tuple<string, Action>[] acts)
        {
            if (showEffect)
            {
                HeBianGu.General.WpfControlLib.Message.Instance.BeginDefaultBlurEffect(true);
            ...
            if (showEffect)
            {
                HeBianGu.General.WpfControlLib.Message.Instance.BeginDefaultBlurEffect(false);
        ...
        /// <summary> 显示窗口 </summary>
        public static int ShowDialogWith(string messge, string title = null, bool showEffect = false, params Tuple<string, Action<IMessageDialogWindow>>[] acts)
        {
            if (showEffect)
            {
                HeBianGu.General.WpfControlLib.Message.Instance.BeginDefaultBlurEffect(true);
            ...
            else
            {
                ...
                foreach (Tuple<string, Action<IMessageDialogWindow>> item in acts)
                {
                    Button f = new Button();
                    f.Content = item.Item1;
                    //f.Width = double.NaN;
