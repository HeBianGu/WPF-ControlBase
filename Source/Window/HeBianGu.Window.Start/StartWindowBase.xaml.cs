...
namespace HeBianGu.Window.Start
{
    public partial class StartWindowBase : WindowBase, IStartWindow
    {
        ...
        public StartWindowBase()
        {
            ...
            if (string.IsNullOrEmpty(StartSetting.Instance.Copyright))
            {
                StartSetting.Instance.Copyright = Assembly.GetEntryAssembly().GetCustomAttribute<AssemblyCopyrightAttribute>()?.Copyright;
            }

            if (string.IsNullOrEmpty(StartSetting.Instance.Title))
            {
                StartSetting.Instance.Title = Assembly.GetEntryAssembly().GetCustomAttribute<AssemblyTitleAttribute>()?.Title;
            }

            this.Copyright = StartSetting.Instance.Copyright;

            this.Title = StartSetting.Instance.Title;
