...
namespace HeBianGu.General.WpfControlLib
{
    internal static class TreeHelper
    {
        public static double GetVisibleWidth(FrameworkElement element, UIElement parent)
        {
            ...
            Point location = element.TransformToAncestor(parent).Transform(new Point(0, 0));

            int width = (int)Math.Floor(element.ActualWidth);
            IInputElement hitTest = parent.InputHitTest(new Point(location.X + width, location.Y));
