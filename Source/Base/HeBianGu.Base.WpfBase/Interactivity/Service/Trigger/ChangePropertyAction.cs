...
namespace HeBianGu.Base.WpfBase
{
    ...
    public class ChangeAttachPropertyAction : ChangePropertyAction
    {
        public Type AttachType
        {
            get { return (Type)GetValue(AttachTypeProperty); }
            set { SetValue(AttachTypeProperty, value); }
        }

        // Using a DependencyProperty as the backing store for MyProperty.  This enables animation, styling, binding, etc...
        public static readonly DependencyProperty AttachTypeProperty =
            DependencyProperty.Register("AttachType", typeof(Type), typeof(ChangeAttachPropertyAction), new FrameworkPropertyMetadata(default(Type), (d, e) =>
             {
                 ChangeAttachPropertyAction control = d as ChangeAttachPropertyAction;
        ...
        protected override void Invoke(object parameter)
        {
            ...
            var methed = this.AttachType.GetMethod("Set" + this.PropertyName);

            Type targetType = this.Target.GetType();
            //PropertyInfo propertyInfo = targetType.GetProperty(this.PropertyName);
            //this.ValidateProperty(prperty);

            var pType = methed.GetParameters()[1].ParameterType;



            object newValue = this.Value;
            TypeConverter converter = TypeConverterHelper.GetTypeConverter(pType);
            ...
            try
            {
                if (this.Value != null)
                {
                    ...
                    else
                    {
                        // Try asking the value if it can convert itself to the target property
                        converter = TypeConverterHelper.GetTypeConverter(this.Value.GetType());
                        if (converter != null && converter.CanConvertTo(pType))
                        {
                            newValue = converter.ConvertTo(
                                context: null,
                                culture: CultureInfo.InvariantCulture,
                                value: this.Value,
                                destinationType: pType);
                        }
                    }
                }
                ...
                methed.Invoke(null, new object[] { this.Target, newValue });
            }
            catch (FormatException e)
            {
                innerException = e;
