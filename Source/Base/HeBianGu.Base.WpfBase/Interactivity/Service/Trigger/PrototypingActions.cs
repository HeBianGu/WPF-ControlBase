...
namespace HeBianGu.Base.WpfBase
{
    ...
    public abstract class PrototypingActionBase : TriggerAction<DependencyObject>
    {
        ...
        {
            System.Collections.Generic.IEnumerable<UserControl> userControlAncestors = this.AssociatedObject.GetSelfAndAncestors().OfType<UserControl>();

            // Find the first screen up the visual tree, so that we can invoke state on the proper UserControl
            UserControl screen = userControlAncestors.FirstOrDefault(userControl => InteractionContext.IsScreen(userControl.GetType().FullName));
