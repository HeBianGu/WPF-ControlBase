...
namespace HeBianGu.Systems.Upgrade
{
    ...
    public static class AutoUpdater
    {
        ...
        /// <param name="myAssembly">Assembly to use for version checking.</param>
        public static async void Start(string appCast, Assembly myAssembly = null)
        {
            try
            {
            ...
            if (!Running && _remindLaterTimer == null)
            {
                ...
                if (Synchronous)
                {
                    try
                    {
                        var result = CheckUpdate(assembly, appCast);
                ...
                else
                {
                    using (var backgroundWorker = new BackgroundWorker())
                    {
                        backgroundWorker.DoWork += (_, args) =>
                        {
                            ...
                            args.Result = CheckUpdate(mainAssembly, appCast);
                        };
        ...
        public static void ShowUpdateForm(UpdateInfoEventArgs args)
        {
            ...
            UpgradeWindow.BeginUpgrade(args.CurrentVersion.ToString(), args.DownloadURL, args.ChangelogURL?.Split(';'));
        }
