    ...
    public class SnackbarMessageQueue : ISnackbarMessageQueue, IDisposable
    {
        ...
        private class DurationMonitor
        {
                ...
                WaitHandle ceaseWaitHandle)
            {
                ...
                TimeSpan granularity = TimeSpan.FromMilliseconds(200);
            ...
            object actionArgument, bool promote, bool neverConsiderToBeDuplicate)
        {
            ...
            SnackbarMessageQueueItem snackbarMessageQueueItem = new SnackbarMessageQueueItem(content, actionContent, actionHandler,
        ...
        private async void PumpAsync()
        {
            while (!_isDisposed)
            {
                int eventId = WaitHandle.WaitAny(new WaitHandle[] { _disposedEvent, _messageWaitingEvent });
                ...
                //find a target
                Snackbar snackbar = await FindSnackbar(exemplar.Dispatcher);
        ...
        private DispatcherOperation<Snackbar> FindSnackbar(Dispatcher dispatcher)
        {
            return dispatcher.InvokeAsync(() =>
            {
                return _pairedSnackbars.FirstOrDefault(sb =>
                {
                    ...
                    Window window = Window.GetWindow(sb);
            ...
            await Task.Run(async () =>
                {
                    ...
                    ManualResetEvent actionClickWaitHandle = new ManualResetEvent(false);
                    ...
                    ManualResetEvent durationPassedWaitHandle = new ManualResetEvent(false);
                ...
                .ContinueWith(t =>
                {
                    ...
                    Exception exc = t.Exception.InnerExceptions.FirstOrDefault() ?? t.Exception;
